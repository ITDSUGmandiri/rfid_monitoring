...
class Crud extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => ADMIN_NAMESPACE_URL  . '/crud/index/',
	...
	public function edit($id)
	{
		...
		$this->load->model('database/model_database');
	...
	public function save_crud()
	{
		if ($this->form_validation->run()) {
			...
			$this->data = [
				...
				'use_modal'					=> $this->input->post('crud_modal') == 'yes',
			...
			$builder_list = $this->parser->parse($template_crud_path . 'builder_data_table', $this->data, true);
			write_file($view_path . $table_name . '_data_table.php', $builder_list);
			...
			$save_data = [
				...
				'crud_modal' 		=> $this->input->post('crud_modal'),
			];
			...
			if ($this->input->post('save_type') == 'stay') {
				...
				$this->response['message'] = cclang('success_save_data_stay', [
					admin_anchor('/crud', ' Go back to list'),
					admin_anchor('/' . $this->input->post('table_name'), ' View')
				]);
			} else {
				set_message(
				...
				$this->response['success'] = true;
				$this->response['redirect'] = admin_site_url('/crud/edit/' . $id_crud);
			}
		} else {
	...
	public function delete($id = null)
	{
		...
		redirect(ADMIN_NAMESPACE_URL . '/crud');
	}
	...
	public function save_action()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($save_action) {
				...
				$this->data['redirect'] = admin_base_url('/keys');
			}
		} else {
			$this->data['success'] = false;
