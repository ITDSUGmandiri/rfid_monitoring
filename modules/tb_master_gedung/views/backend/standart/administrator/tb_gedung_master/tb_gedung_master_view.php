...
<?php if ($this->session->flashdata('nulldata')) { ?>
   <script>
      Swal.fire({
         icon: "warning",
         title: "Oops, Detail Gedung tidak ada",
         text: "Kode Gedung tidak ditemukan, silahkan perbarui data Aset!",
      }).then(okay => {
         if (okay) {
            window.history.go(-1);
         }
         ...
         <?php foreach ($tb_master_gedung as $value): ?>
            <div class="row">
               <div class="col-12 col-sm-5">
                  <h3 class="d-inline-block d-sm-none"><?= $value->gedung; ?></h3>
                  <div class="col-12">
                     <img id="myImg" src="<?= base_url('uploads'); ?>/Gedung/<?= $value->image_uri ?>" class="product-image" alt="Area Image">
                  </div>
