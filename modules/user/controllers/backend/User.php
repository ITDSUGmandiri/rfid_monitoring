...
class User extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => ADMIN_NAMESPACE_URL  . '/user/index/',
			...
			if ($save_user) {
				...
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->response['message'] = cclang('success_save_data_stay', [
						admin_anchor('/user/edit/' . $save_user, 'Edit User'),
						admin_anchor('/user', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->response['success'] = true;
					$this->response['redirect'] = admin_site_url('/user');
				}
			} else {
	...
	public function edit_profile_save($id)
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($save_user) {
				...
				$this->data['id'] 	   = $id;
				$this->data['message'] = 'Your data has been successfully updated into the database. ' . admin_anchor('/user', ' Go back to list');
			} else {
		...
		if (!$user) {
			...
			if (!empty($user->avatar)) {
				$result[] = [
					...
					'name' 					=> $user->avatar,
					'uuid' 					=> $user->id,
					'deleteFileEndpoint' 	=> admin_base_url('/user/delete_avatar_file'),
