...
function domo(){
   ...
   $('*').bind('keydown', 'Ctrl+a', function() {
       window.location.href = ADMIN_BASE_URL + '/Tb_master_transaksi/add';
       return false;
   });
               ...
               <div class="box box-widget widget-user-2">
                  <div class="widget-user-header ">
                     <div class="row pull-right">
                        ...
                        <a class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', [cclang('tb_master_transaksi')]); ?>  (Ctrl+a)" href="<?=  admin_site_url('/tb_master_transaksi/add'); ?>"><i class="fa fa-plus-square-o" ></i> <?= cclang('add_new_button', [cclang('tb_master_transaksi')]); ?></a>
                        <?php }) ?>
                        <?php is_allowed('tb_master_transaksi_export', function(){?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> <?= cclang('tb_master_transaksi') ?> " href="<?= admin_site_url('/tb_master_transaksi/export?q='.$this->input->get('q').'&f='.$this->input->get('f')); ?>"><i class="fa fa-file-excel-o" ></i> <?= cclang('export'); ?> XLS</a>
                        <?php }) ?>
                                                <?php is_allowed('tb_master_transaksi_export', function(){?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> pdf <?= cclang('tb_master_transaksi') ?> " href="<?= admin_site_url('/tb_master_transaksi/export_pdf?q='.$this->input->get('q').'&f='.$this->input->get('f')); ?>"><i class="fa fa-file-pdf-o" ></i> <?= cclang('export'); ?> PDF</a>
                        <?php }) ?>
                                             </div>
                  ...
                  <form name="form_tb_master_transaksi" id="form_tb_master_transaksi" action="<?= admin_base_url('/tb_master_transaksi/index'); ?>">
    ...
    $(document).on('click', '#apply', function(){
      ...
      if (bulk.val() == 'delete') {
         swal({
          ...
          function(isConfirm){
            if (isConfirm) {
               document.location.href = ADMIN_BASE_URL + '/tb_master_transaksi/delete?' + serialize_bulk;      
            }
          });
