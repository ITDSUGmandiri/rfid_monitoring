...
if (!function_exists('admin_site_url')) {
	function admin_site_url($url = null)
	{
		$use_slash = substr($url, 0, 1) == '/' ? '' : '/';
		return site_url(ADMIN_NAMESPACE_URL . $use_slash . $url);
	}
}
if (!function_exists('replace_admin_url')) {
	function replace_admin_url($url = null)
	{
		return str_replace(['administrator', '{admin_url}'], ADMIN_NAMESPACE_URL, $url);
	}
}
if (!function_exists('admin_base_url')) {
	function admin_base_url($url = null)
	{
		$use_slash = substr($url, 0, 1) == '/' ? '' : '/';
		return base_url(ADMIN_NAMESPACE_URL . $use_slash . $url);
	}
}

if (!function_exists('admin_anchor')) {
	function admin_anchor(
		$uri = '',
		$title = '',
		$attributes = ''

	) {
		return anchor(ADMIN_NAMESPACE_URL . '/' . $uri, $title, $attributes);
	}
}
...
if (!function_exists('db_get_all_data_reza')) {
	function db_get_all_data_reza($table_name = null, $where = false, $join = null, $join_on = null, $join_type = 'inner')
	{
		$ci = &get_instance();
		if ($where) {
			$ci->db->where($where);
		...
		if ($join && $join_on) {
			$ci->db->join($join, $join_on, $join_type);
		}
		$query = $ci->db->get($table_name);
		...
		$display_menu_dropdown = function ($row = null) {
			return '<div class="dropdown  pull-right">
					    ...
					    <ul class="dropdown-menu">
					      <li><a class="remove-data" href="javascript:void()" data-href="' . admin_site_url('menu/delete/' . $row->id) . '"><i class="fa fa-trash btn-action"></i> <span class="text-danger">Delete</span></a></li>
					      <li><a href="' . admin_site_url('menu/edit/' . $row->id) . '"><i class="fa fa-pencil btn-action"></i> Edit</a></li> 
					      ' . ($row->type != 'label' ? '
					      <li><a href="' . admin_site_url('menu/add/' . app()->uri->segment(4) . '?parent=' . $row->id) . '"><i class="fa fa-level-down btn-action"></i> Add child</a></li>
					      ' : '') . '
					    </ul>
					  </div>';
		};
				...
				if (isset($ext->loader)) {
					if (isset($ext->routes)) {
						foreach ($ext->routes as $route) {
							...
							$route = str_replace('administrator', ADMIN_NAMESPACE_URL, $route);
...
if (!function_exists('cclang')) {
	function cclang($langkey = null, $params = [])
	{
		...
		if (!$lang) {
			return ucwords(str_replace('_', ' ', $langkey));
		}
...
if (!function_exists('webPageUrl')) {
	function webPageUrl($page, $params = [])
	{
		...
		return admin_site_url('web-page?' . http_build_query($params));
	}
}
...
if (!function_exists('get_user_first_group')) {
	function get_user_first_group($user_id = null)
	{
		app()->load->library('aauth');
		foreach (app()->aauth->get_user_groups($user_id) as $row) {
			if (strtolower($row->name) != 'default') {
				return $row;
			}
		}
...
if (!function_exists('ccencrypt')) {
	function ccencrypt($string)
	{
		$iv_length = openssl_cipher_iv_length(CIP_CIPERING);
		$options = 0;
...
if (!function_exists('get_user_sesi_pegawai')) {
	function get_user_sesi_pegawai()
	{
		$ci = &get_instance();
		return $ci->session->userdata('sesi_pegawai');
	}
}

if (!function_exists('report_builder')) {
	function report_builder($report_id, $row_id)
	{
		return admin_base_url('report/preview/' . $report_id . '/' . $row_id);
	}
}


function get_input_text_type($input, $option)
{
	?>
	<div class="form-group <?= @$option['wrapper_class'] ?>  ">
		<label for="<?= $input; ?>" class="col-sm-2 control-label"><?= ucwords(clean_snake_case($option['label'])); ?>
			<?php if (app()->crud_builder->getFieldValidation($input, 'required')) { ?><i class="required">*</i>
			<?php } ?></label>
		<div class="col-sm-8">
			...
			<small class="info help-block">
				<?= (isset($option['auto_generated_helpblock']) and $option['auto_generated_helpblock'] == 'yes') ? app()->crud_builder->parseValidationFile($input) : $option['help_block']; ?></small>
		</div>
	</div>
<?php
}
