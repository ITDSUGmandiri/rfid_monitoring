...
async function updateFlagAlarmDeras() {
   ...
   $("#your_table_id tbody tr").each(function (index, tr) {
      ...
      let kode_epc = $(tr).find('td:eq(5)').data('kode_epc').trim();
                ...
                <h3 style="text-decoration: underline;">Detail Perbaikan</h3>
                     ...
                     <!-- <div class="form-group">
                        <div class="row">
                           ...
                           <div class="col-sm-8" style="padding-top: 7px;">
                              <?= _ent($tb_master_transaksi->status_transaksi); ?>
                           </div>
                        </div>
                     </div> -->
               ...
               <div class="view-nav text-center">
                  ...
                  <?php if ($show_batal_button): ?>
                     <a class="btn btn-flat btn-danger" id="btn_batal" href="javascript:void(0);" data-id="<?= $id; ?>">
                        <i class="fa fa-times"></i> <?= cclang('perbaikan_batal', ['Perbaikan']); ?>
                     </a>
                  <?php endif; ?>
                  </div>
...
try {
    const response = await $.ajax({
        url: `${ADMIN_BASE_URL}/perbaikan/get_search_aset?id=${id}`,
        type: 'GET',
        dataType: 'json',
