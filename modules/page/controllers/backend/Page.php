...
class Page extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => ADMIN_NAMESPACE_URL  . '/page/index/',
	...
	public function admin()
	{
		...
		if ($body === NULL) {
			redirect(ADMIN_NAMESPACE_URL . '/page/404', 'refresh');
		}
	}
			...
			if ($save_page) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_save_data_stay', [
						admin_anchor('/page/edit/' . $save_page, 'Edit Page'),
						admin_anchor('/page', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = admin_base_url('/page');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	public function delete($id = null)
	{
		...
		redirect(ADMIN_NAMESPACE_URL . '/page');
	}
	...
	public function register_unparse_html($page)
	{
		...
		$this->cc_page_element->registerUnParseHtml(function ($html) {
			return preg_replace('/<div class=".*(.element-info-top).*".*\>/i', '', $html);
		});
	...
	public function get_html()
	{
		...
		$content = $this->curl->get($url)->body;
	...
	public function get_image_file($id)
	{
		...
		echo $this->get_file([
			...
			'primary_key'       => 'id',
			'upload_path'       => 'uploads/page/',
			'delete_endpoint'   => ADMIN_NAMESPACE_URL  .  '/page/delete_image_file'
		]);
	}
