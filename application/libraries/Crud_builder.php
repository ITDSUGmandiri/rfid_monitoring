...
class Crud_builder
{
	...
	public function validateAll()
	{
		...
		foreach ($this->crud as $contains) {
			...
			if (isset($contains[$field_name]['label']) and $contains[$field_name]['label'] == null) {
				$this->errors[] = 'The ' . ucwords(clean_snake_case($field_name)) . ' field label can\'t be empty.';
				return $this;
			}
			...
			if (in_array($input_type, $list_relation_type)) {
				if (
					empty($contains[$field_name]['relation_table'])
			...
			if (isset($contains[$field_name]['validation']['rules'])) {
				foreach ($contains[$field_name]['validation']['rules'] as $rule => $value) {
					...
					if (in_array($rule, ['max_width', 'max_height'])) {
						if (
							!isset($contains[$field_name]['validation']['rules']['max_width'])
	 ...
	 * @return	array
	 */
	public function getFieldShowInColumn($withOption = false)
	{
		$fields = [];
		foreach ($this->crud as $contains) {
	 ...
	 * @return	mixed array | string
	 */
	public function getFieldChain($field_name = null)
	{
		$fields = [];
		foreach ($this->crud as $contains) {
			$field = array_keys($contains)[0];
			if (isset($contains[$field]['chain_field']) and !empty($contains[$field]['chain_field'])) {
				if (isset($contains[$field]['chain_field']) and !empty($contains[$field]['chain_field'])) {
					if (isset($contains[$field]['chain_field_eq']) and !empty($contains[$field]['chain_field_eq'])) {
						$fields[$field] = [
							'chain_field_eq' => $contains[$field]['chain_field_eq'],
							'chain_field' => $contains[$field]['chain_field'],
							'relation_table' => $contains[$field]['relation_table'],
							'relation_value' => $contains[$field]['relation_value'],
							'relation_label' => $contains[$field]['relation_label'],
						];
	...
	/**
	 * Check field is custom setting
	 *
	 * @access	public
	 * @param String $field_name
	 * @return	mixed array | string
	 */
	public function getFieldCustomSetting($field_name = null, $setting_name = 'custom_condition')
	{
		$fields = [];
		foreach ($this->crud as $contains) {
			$field = array_keys($contains)[0];

			if (isset($contains[$field][$setting_name])) {
				$fields[$field] = $contains[$field][$setting_name];
			}
		}
	 ...
	 * @return	array
	 */
	public function getFunctionBody($input, $type = 'post')
	{
		if ($type == 'post') {
			$string = $this->ci->input->post($input);
		} else {
			$string = $input;
		}

		$result = preg_match_all('/function\s+(?<name>\w+)\s*\((?<param>[^\)]*)\)\s*(?<body>\{(?:[^{}]+|(?&body))*\})/', (string)$string, $matches);

		$functions = [];

		$lit = 0;
		foreach ($matches['name'] as $match) {
			$functions[$match] = $matches['body'][$lit];
			$lit++;
		}
