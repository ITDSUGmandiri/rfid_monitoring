...
class Group extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => ADMIN_NAMESPACE_URL  . '/group/index/',
			...
			if ($save_group) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_save_data_stay', [
						admin_anchor('/group/edit/' . $save_group, 'Edit Group'),
						admin_anchor('/group', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = admin_base_url('/group');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
