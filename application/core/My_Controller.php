...
trait main_validation
{
    ...
    public function edit_is_unique($str, $field)
    {
        ...
        if ($return == false) {
            $this->form_validation->set_message('edit_is_unique', 'The %s field must contain a unique value.');
        }
        return $return;
    }
}
...
class Admin extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->limit_page = get_option('limit_pagination', 10);
    }
     ...
     * @return JSON
     */
    public function renderMobile($view = '', $data = array(), $bool = FALSE)
    {
        $this->template->enable_parser(false);
        $this->template->set_partial('content', $view, $data);
        $this->template->build('backend/standart/main_layout_mobile', $data);
    }
    ...
    public function get_file($data = [])
    {
        $default = [
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/blog/',
            'delete_endpoint'   => ADMIN_NAMESPACE_URL . '/blog/delete_image_file'
        ];
...
class Front extends MY_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        if (installation_complete()) {
            $this->active_theme = get_option('active_theme', 'cicool');
