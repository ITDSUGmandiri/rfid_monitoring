<?php foreach($tb_area_masters as $tb_area_master): ?>
    <tr>
                <td width="5">
            <input type="checkbox" class="flat-red check" name="id[]" value="<?= $tb_area_master->id; ?>">
        </td>
                
        <td><span class="list_group-id_area"><?= _ent($tb_area_master->id_area); ?></span></td> 
        ...
        <td width="200">
            ...
            <a href="javascript:void(0);" data-href="<?= admin_site_url('/tb_area_master/delete/' . $tb_area_master->id); ?>" class="label-default remove-data"><i class="fa fa-close"></i> <?= cclang('remove_button'); ?></a>
            <?php }) ?>
