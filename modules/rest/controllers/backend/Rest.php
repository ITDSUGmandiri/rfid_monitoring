...
class Rest extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => ADMIN_NAMESPACE_URL  . '/rest/index/',
	...
	public function save_rest()
	{
		if ($this->form_validation->run()) {
			...
			$this->data = [
				...
				'field_relations'			=> $this->crud_builder->getFieldRelation(),
			...
			if ($this->input->post('save_type') == 'stay') {
				...
				$this->response['message'] =
					cclang('success_save_data_stay', [
						admin_anchor('/rest', ' Go back to list'),
						admin_anchor('/rest/view/' . $id_rest, ' View')
					]);
			} else {
				set_message(
				...
				$this->response['success'] = true;
				$this->response['redirect'] = admin_site_url('/rest');
			}
		} else {
	...
	public function delete($id = null)
	{
		...
		redirect(ADMIN_NAMESPACE_URL . '/rest');
	}
	...
	{
		$this->data['html'] = $this->load->view('backend/standart/administrator/rest/rest_list_field.php', ['table' => $table], true);
		$this->data['success'] = true;
	...
	{
		$this->data['html'] = $this->load->view('backend/standart/administrator/rest/rest_list_field_label.php', ['table' => $table], true);
		$this->data['success'] = true;
