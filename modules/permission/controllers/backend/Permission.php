...
class Permission extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => ADMIN_NAMESPACE_URL  . '/permission/index/',
			...
			if ($save_permission) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->response['message'] = cclang('success_save_data_stay', [
						admin_anchor('/permission/edit/' . $save_permission, 'Edit Permission'),
						admin_anchor('/permission', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->response['success'] = true;
					$this->response['redirect'] = admin_site_url('/permission');
				}
			} else {
