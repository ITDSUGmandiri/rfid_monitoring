...
class Model_crud extends MY_Model
{
	...
	public function get_crud_action($id)
	{
		$result = $this->db->get_where('crud_action', ['crud_id' => $id])->result();
	...
	public function get_crud_field_configuration($id)
	{
		$configuration = [];

		$result = $this->db->get_where('crud_field_configuration', ['crud_id' => $id])->result();

		foreach ($result as $row) {
			$configuration[$row->crud_field_id][$row->group_config][] = $row;
		}
	...
	public function get_crud_chained($id)
	{
		$result = $this->db->get_where('crud_input_chained', ['crud_id' => $id])->result();
		$items = [];
		foreach ($result as $item) {
			$items[$item->crud_field_id] = $item;
		}
