...
class Migrate extends Admin
{
	...
	{
		$info = $this->db->get_where('migrations')->row();

		for ($i = $info->version; $i <= $info->latest_file_version; $i++) {
			echo $i . ' ';
			$this->migration->version($i);
		}
		echo 'Successfully!';
	}
	...
	public function index($file_name = '')
	{
		...
		$migration_file_table = $this->input->get('table');
		...
		$msg =  'Migration File '  .  $file_full_name  .  ' successfully created!';
	...
	public function latest()
	{
		if ($this->migration->latest() === FALSE) {
			set_message(cclang('migrate_error'), 'error');

			redirect_back();
		}

		set_message(cclang('migrate_successfull'), 'success');
	...
	public function rollback($version = 0)
	{
		if ($this->migration->version(abs($version)) === FALSE) {
			set_message($this->migration->error_string(), 'error');
			redirect_back();
		}

		set_message("Migrated to the version - "  . $version, 'success');
		redirect_back();
	}
