...
<section class="content-header">
    <h1>
        <?= ucwords($subject); ?>
        <small>{php_open_tag_echo} cclang('update'); {php_close_tag} <?= ucwords(clean_snake_case($subject)); ?></small>
    </h1>
    <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class=""><a href="{php_open_tag_echo} admin_site_url('/manage-form/{table_name}'); {php_close_tag}"><?= ucwords(clean_snake_case($subject)); ?></a></li>
        <li class="active">{php_open_tag_echo} cclang('update'); {php_close_tag}</li>
    </ol>
</section>
<!-- Main content -->
            ...
            <div class="box box-warning">
                <div class="box-body ">
                    ...
                    <div class="box box-widget widget-user-2">
                        ...
                        {php_open_tag_echo} form_open(admin_base_url('/manage-form/{table_name}/edit_save/'.$this->uri->segment(5)), [
                        ...
                        <?php foreach ($field_all as $input => $option) : ?>
                            ...
                            ?><div class="form-group ">
                                    ...
                                    <div class="col-sm-2">
                                        <select class="form-control chosen chosen-select-deselect" name="<?= $option['input_name']; ?>" id="<?= $option['input_name']; ?>" data-placeholder="Select <?= ucwords(clean_snake_case($option['input_label'])); ?>" <?= $this->crud_builder->parseAttributes((isset($option['custom_attributes']) ? $option['custom_attributes'] : [])); ?>>
                                            <option value=""></option>
                                            {php_open_tag} for ($i = 1970; $i < date('Y')+100; $i++){ {php_close_tag} <option {php_open_tag_echo} $i==${table_name}-><?= $option['input_name']; ?> ? 'selected' : ''; {php_close_tag} value="{php_open_tag_echo} $i; {php_close_tag}">{php_open_tag_echo} $i; {php_close_tag}</option>
                        ...
                        <div class="row-fluid col-md-7">
                            ...
                            <span class="loading loading-hide">
                                ...
                                <i>{php_open_tag_echo} cclang('loading_field_data'); {php_close_tag}</i>
                            </span>
                        </div>
...
<script>
    $(document).ready(function() {
        <?php foreach ($address_map as $input) : ?>$('#<?= $option['input_name']; ?>').addressPickerByGiro({
            distanceWidget: true
        });
    <?php
        endforeach;
    ...
    $('#btn_cancel').click(function() {
        swal({
            ...
            function(isConfirm) {
                if (isConfirm) {
                    window.location.href = ADMIN_BASE_URL + '/manage-form/{table_name}';
                }
            });
    ...
    ?>
        ...
        $('#{table_name}_<?= $field_all[$input]['input_name']; ?>_galery').fineUploader({
            ...
            request: {
                endpoint: ADMIN_BASE_URL + '/manage-form/{table_name}/upload_<?= $field_all[$input]['input_name']; ?>_file',
                params: params
            },
            deleteFile: {
                enabled: true,
                endpoint: ADMIN_BASE_URL + '/manage-form/{table_name}/delete_<?= $field_all[$input]['input_name']; ?>_file'
            },
            thumbnails: {
                placeholders: {
            ...
            session: {
                endpoint: ADMIN_BASE_URL + '/manage-form/{table_name}/get_<?= $field_all[$input]['input_name']; ?>_file/{php_open_tag_echo} ${table_name}->{primary_key}; {php_close_tag}',
            ...
            callbacks: {
                ...
                onSubmit: function(id, name) {
                    ...
                    $.get(ADMIN_BASE_URL + '/manage-form/{table_name}/delete_<?= $field_all[$input]['input_name']; ?>_file/' + uuid);
                },
