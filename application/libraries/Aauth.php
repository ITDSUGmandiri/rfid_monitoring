...
class Aauth
{
	 ...
	 */
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL, $set_userdata = TRUE)
	{
	 ...
	 */
	public function reset_password($ver_code, $pass)
	{
	 ...
	 * @return bool Update fails/succeeds
	 */
	public function update_user($user_id, $email = FALSE, $pass = FALSE, $username = FALSE, $optional_data = [])
	{
	...
	public function is_allowed($perm_par, $user_id = FALSE)
	{
		...
		if ($this->CI->cc_app->eventListen('auth_allowed_permission', [$perm_par]) === false) {
			var_dump($this->CI->cc_app->eventListen('auth_allowed_permission', [$perm_par]));
			exit();
			return false;
		}
