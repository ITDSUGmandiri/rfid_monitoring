<script type="text/javascript">
   function domo() {
      ...
      $('*').bind('keydown', 'Ctrl+a', function() {
         window.location.href = ADMIN_BASE_URL + '/Tb_room_master/add';
         return false;
      });
                  ...
                  <div class="widget-user-header ">
                     <div class="row pull-right">
                        <?php is_allowed('tb_room_master_add', function () { ?>
                           <a class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', [cclang('tb_room_master')]); ?>  (Ctrl+a)" href="<?= admin_site_url('/tb_room_master/add'); ?>"><i class="fa fa-plus-square-o"></i> <?= cclang('add_new_button', [cclang('tb_room_master')]); ?></a>
                        <?php }) ?>
                        <?php is_allowed('tb_room_master_export', function () { ?>
                           <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> <?= cclang('tb_room_master') ?> " href="<?= admin_site_url('/tb_room_master/export?q=' . $this->input->get('q') . '&f=' . $this->input->get('f')); ?>"><i class="fa fa-file-excel-o"></i> <?= cclang('export'); ?> XLS</a>
                        <?php }) ?>
                        <?php is_allowed('tb_room_master_export', function () { ?>
                           <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> pdf <?= cclang('tb_room_master') ?> " href="<?= admin_site_url('/tb_room_master/export_pdf?q=' . $this->input->get('q') . '&f=' . $this->input->get('f')); ?>"><i class="fa fa-file-pdf-o"></i> <?= cclang('export'); ?> PDF</a>
                        <?php }) ?>
                     </div>
                        ...
                        <div class="col-md-8">
                           ...
                           <div class="col-sm-3 padd-left-0 ">
                              <select type="text" class="form-control chosen chosen-select" name="f" id="field">
                                 ...
                                 <option <?= $this->input->get('f') == 'kode_room' ? 'selected' : ''; ?> value="kode_room">Kode Room</option>
                           ...
                           <div class="col-sm-1 padd-left-0 ">
                              <a class="btn btn-default btn-flat" name="reset" id="reset" value="Apply" href="<?= admin_base_url('/tb_room_master'); ?>" title="<?= cclang('reset_filter'); ?>">
...
<script>
   $(document).ready(function() {
      ...
      $(document).on('click', '#apply', function() {
         ...
         var bulk = $('#bulk');
         var serialize_bulk = $('#form_tb_room_master').serialize();
         ...
         if (bulk.val() == 'delete') {
            swal({
               ...
               function(isConfirm) {
                  if (isConfirm) {
                     document.location.href = ADMIN_BASE_URL + '/tb_room_master/delete?' + serialize_bulk;
                  }
               });
      ...
      initSortableAjax('tb_room_master', $('table.dataTable'));
   }); /*end doc ready*/
</script>