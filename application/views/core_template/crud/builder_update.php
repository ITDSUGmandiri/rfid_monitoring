            ...
            <div class="box box-warning">
                <div class="box-body ">
                    <div class="box box-widget widget-user-2">
                        ...
                        {php_open_tag_echo} form_open(admin_base_url('/{table_name}/edit_save/'.$this->uri->segment(4)), [
                        'name' => 'form_{table_name}_edit',
                        'class' => 'form-horizontal form-step',
                        'id' => 'form_{table_name}_edit',
                        'method' => 'POST'
                        ]); {php_close_tag}
                        ...
                        foreach ($this->crud_builder->getFieldShowInUpdateForm(true) as $input => $option) : ?>
                            <?php if (in_array($option['input_type'], $this->crud_builder->getFieldNotShowInUpdateForm())) continue; ?><?php if (isset($option['configs']['stepper'])) : $step_title = $option['configs']['stepper']['title'];
                                                                                                                                            if (count($stepper)) {
                                                                                                                                        ?>
</section><?php  }
                                                                                                                                            $stepper[$option['input_type']] = $step_title;  ?><h3><?= $step_title ?></h3>
<section>
<?php endif ?>
...
<?php if ($option['input_type'] == 'input') { get_input_text_type($input, $option);?>
...
<script>
    ...
    var form_<?= $table_name ?> = $('#form_{table_name}_edit');
        ...
        .done(function(res) {
            ...
            if (res.success) {
                ...
                if (use_ajax_crud == true) {
                    ...
                    var url = BASE_URL + ADMIN_NAMESPACE_URL + '/{table_name}/index/?ajax=1'
                    reloadDataTable(url);
                }
    ...
    <?php foreach ($fine_upload as $input) : ?>
        ...
        ?>
            ...
            $('#{table_name}_<?= $input; ?>_galery').fineUploader({
                ...
                session: {
                    endpoint: ADMIN_BASE_URL + '/{table_name}/get_<?= $input; ?>_file/{php_open_tag_echo} ${table_name}->{primary_key}; {php_close_tag}',
