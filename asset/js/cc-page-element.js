...
CcPageElement.prototype.displayComponentItem = function (component_name) {
	...
	if (typeof this.component_item[component_name] != undefined) {
		$.each(this.component_item[component_name], function (idx, component_item) {
			...
			var additional = '<div class="element-info-top not-editable" style="" contentEditable="false" >' +
				node_name +
...
CcPageElement.addField({
	...
	'onReady': function (field) {
		...
		$('#image_galery').fineUploader({
			...
			request: {
				endpoint: ADMIN_BASE_URL + '/page/upload_image_file',
				params: params
			},
			deleteFile: {
				enabled: true, // defaults to false
				endpoint: ADMIN_BASE_URL + '/page/delete_image_file',
			},
			thumbnails: {
				placeholders: {
			...
			callbacks: {
				...
				onSubmit: function (id, name) {
					...
					$.get(ADMIN_BASE_URL + '/page/delete_image_file/' + uuid);
				}
			}
		});
