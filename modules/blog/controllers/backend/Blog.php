...
class Blog extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => ADMIN_NAMESPACE_URL  . '/blog/index/',
			'total_rows'   => $this->data['blog_counts'],
		...
		if ($this->form_validation->run()) {
			...
			if (count((array) $this->input->post('blog_image_name'))) {
				foreach ((array) $_POST['blog_image_name'] as $idx => $file_name) {
					...
					rename(
						FCPATH . 'uploads/tmp/' . $_POST['blog_image_uuid'][$idx] . '/' .  $file_name,
			...
			if ($save_blog) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_save_data_stay', [
						admin_anchor('/blog/edit/' . $save_blog, 'Edit Blog'),
						admin_anchor('/blog', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = admin_base_url('/blog');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	public function edit_save($id)
	{
		...
		if ($this->form_validation->run()) {
			...
			$save_data['image'] = implode(',', (array)$listed_image);
